...
if ($demonumber) {
  ...
  if ($launch_files[$nb] && $launch_data{$launch_files[$nb]}) {
    ...
    my $pomdirectory = dirname($launch_files[$nb]);
    while (! -f File::Spec->catfile($pomdirectory, "pom.xml")) {
      $pomdirectory = dirname($pomdirectory);
    }
    ...
    foreach my $agent_key (@{$demo->{'*'}{'sections'}}) {
      ...
      if ($agent_name) {
        ...
        if ($pid) {
          ...
          chdir($pomdirectory) || die("$pomdirectory: $!\n");
          print "> cwd: ".getcwd()."\n";
          exec @cmd_line;
          die("exec failed!");
        }
