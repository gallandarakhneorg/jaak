...
import io.sarl.jaak.environment.frustum.SquareTurtleFrustum;
...
import io.sarl.jaak.environment.perception.StandardObjectManipulator;
...
public class EnvironmentModel implements EnvironmentArea {
	...
	@Override
	public synchronized Serializable getTurtleSemantic(int x, int y) {
		TurtleBody body = this.grid.getTurtle(x, y);
		if (body != null) {
			return body.getSemantic();
		}
		return null;
	}
	...
	public Serializable getGroundType(int x, int y) {
		return this.grid.getGroundType(x, y);
	}
	...
	private void computePerceptions() {
		...
		for (RealTurtleBody body : this.bodies.values()) {
						...
						while (iterator.hasNext()) {
							...
							if (this.grid.validatePosition(isWrapped(), true, position) != ValidationResult.DISCARDED) {
								...
								if (semantic != null) {
									...
									relativePosition.sub(position, body.getPosition());
									groundPerceptions.put(relativePosition, semantic);
								}
							}
						}
					}
			...
			body.setPerceptions(bodies, objects, groundPerceptions);
		}
	}
